...
class RecetteListVM : ObservableObject, Subscriber{
    ...
    func receive(_ input: IntentStateRecette) -> Subscribers.Demand {
        ...
        case .ready:
            break
        case .id_createurChanging(_), .nom_recetteChanging(_), .nb_couvertChanging(_), .id_categorieChanging(_), .prix_venteChanging(_), .etapesChanging(_), .recinclusChanging(_), .ingredientsChanging(_), .recetteCreate(_), .imageChanging(_)  :
